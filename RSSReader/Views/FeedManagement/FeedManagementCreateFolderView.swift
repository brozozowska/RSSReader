...
struct FeedManagementCreateFolderView: View {
    @Environment(\.appThemeVariant) private var appThemeVariant
    let presentation: FeedManagementCreateFolderPresentation
        ...
        List {
            ...
            Section {
                TextField(
                    ...
                    text: nameBinding,
                    prompt: Text(FeedManagementLocalization.folderNamePlaceholder)
                )
                ...
                Text(FeedManagementLocalization.folderNamePrompt)
            } footer: {
                Text(FeedManagementLocalization.folderNameFooter)
            }
                ...
                Section {
                    ForEach(presentation.existingFolders) { folder in
                        HStack(alignment: .firstTextBaseline) {
                            ...
                            Text(FeedManagementLocalization.feedCount(folder.feedCount))
                                .font(.footnote)
                                .foregroundStyle(.secondary)
                        }
                    }
                } header: {
                    Text(FeedManagementLocalization.existingFoldersTitle)
                }
            }
