...
- [ ] покрыть тестами `AppSyncBootstrapPreferenceStore` и `NSUbiquitousKeyValueStore` propagation: отдельно зафиксировать приоритет между `NSUbiquitousKeyValueStore` и `UserDefaults`, поведение `synchronize()`, запись bootstrap preference в оба storage и соответствующие diagnostic logs, чтобы sync enablement bootstrap больше не зависел от неявного внешнего состояния;
- [ ] решить судьбу DEBUG-only schema bootstrap guard: либо вынести `CloudKitDevelopmentSchemaBootstrap.didAttemptBootstrapThisLaunch` в явный и тестируемый app-level guard, либо удалить его в пользу более прозрачного orchestration path, чтобы DEBUG bootstrap не зависел от скрытого process-global состояния;
