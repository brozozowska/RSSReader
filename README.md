...
- [ ] `Feed URL Edit Save Fix`: исправить зависание проверки при изменении URL существующей ленты: в `FeedManagementAddFeedState` / `FeedManagementScreenController+AddFeed` зафиксировать state machine `URL changed -> preview loaded for normalized URL -> update command uses that preview -> save completes`, включая случай, когда requested URL и resolved feed URL отличаются; после успешного save обновлять feed details, sidebar projection, icon reload и закрывать feed-management flow без повторного бесконечного preview; покрыть `FeedManagementAddFeedStateTests` и controller tests;
