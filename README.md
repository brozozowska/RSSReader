...
- [ ] `Feed Source Replacement Persistence Reset`: при редактировании существующей ленты и изменении normalized feed URL выполнять source replacement вместо простого metadata update: зафиксировать явный признак `feed URL changed`, атомарно сохранить preview metadata для новой ленты, удалить старые статьи этого `feedID`, почистить orphan `ArticleState`, сбросить старые `iconURL`, `lastETag`, `lastModifiedHeader`, `lastFetchedAt` / `lastSuccessfulFetchAt` и `lastSyncError`, не меняя поведение редактирования одного только display title; покрыть `FeedManagementServiceTests` / repository tests сценарием `t-j.ru -> nplus1.ru`;
- [ ] `Feed Source Replacement Refresh Flow`: после source replacement запускать fresh refresh новой ленты без conditional headers от старого источника, не показывать старые статьи в выбранной ленте между save и завершением загрузки, после результата обновлять feed details, sidebar counts, article list, app badge и feed icon reload; определить, ждёт ли feed-management flow завершения refresh перед закрытием или закрывается с явным loading/empty state; покрыть `FeedManagementScreenController+AddFeed`, app-flow и refresh tests;
- [ ] `Feed Source Replacement Icon Contract`: отдельно зафиксировать icon behavior для смены источника: если preview/refresh новой ленты не даёт подходящую иконку, старая persisted иконка не должна использоваться, а UI должен вернуться к стандартному placeholder/fallback; очистка icon cache не должна ломать загрузку иконок у лент, чей URL не менялся; покрыть tests для feed edit, `FeedIconURLPolicy`, `FeedIconView` и cache clear/refresh сценариев;
