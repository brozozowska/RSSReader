...
- [ ] `Loading Indicator Contract Audit`: пройти оставшиеся loading states после `Shared Loading Indicator Tint And Reuse` и явно зафиксировать, где допустим системный `ProgressView` / UIKit indicator, а где должен использоваться `AppRefreshIndicator`, чтобы UI не возвращался к смешанным indicator semantics перед UI tests;
- [ ] `Test Harness Decomposition`: разложить `RSSReaderTestSupport` на focused builders / fixtures: SwiftData harness, scripted HTTP client, DTO factories, logging doubles и source-management fixtures; тесты должны импортировать общие helpers без неявной зависимости от feed refresh harness;
