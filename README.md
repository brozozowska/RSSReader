...
- [ ] `Current Article Retention Contract`: принять и зафиксировать product rule для статей, которые feed продолжает возвращать: starred articles сохраняются без ограничения, а остальные current/archived articles должны иметь явный time/count budget; обновить settings copy так, чтобы выбранная policy однозначно описывала срок хранения всех XML-materialized статей, а не только исчезнувших из feed;
