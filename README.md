...
- [ ] `Article List Custom Refresh State`: ввести screen-level state для кастомного refresh на `Articles Screen`, отделив состояние pull gesture и видимость индикатора от `ArticleListSession`, `refreshState` загрузки данных и системного `.refreshable`;
