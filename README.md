...
- [ ] `Reader Image Loading Indicator Reuse`: проверить, можно ли заменить стандартный индикатор загрузки картинок в статьях на общий кастомный refresh symbol, который используется при ручном обновлении; отдельно оценить, остаётся ли это небольшой UI-level доработкой или требует изменения image loading state contract в `CachedArticleImageView`;
- [ ] `Preview Feed Refresh Symbol Tint`: сделать цвет кастомного символа загрузки на экране проверки источника визуально вторичным: в `Preview Feed` indicator должен совпадать с серым label рядом с ним и не выделяться чёрным цветом поверх subdued preview-состояния;
- [ ] `Article Title Text Normalization`: нормализовать заголовок статьи до сохранения / показа в списке и reader: декодировать HTML entities вроде `&amp;#132;` / `&amp;#147;`, убрать невидимые управляющие символы из RSS / CDATA payload и сохранить читаемый display title без технических артефактов из feed XML;
- [ ] `Article List Description Line Count`: попробовать увеличить preview description в `ArticleListRowView` с двух до трёх строк и вручную проверить, улучшает ли это сканирование списка без чрезмерного увеличения высоты строк;
