...
- [x] `Source Management Placement Copy Cleanup`: убрать отдельную текстовую секцию `Sidebar Placement` / `Folder Order` из create/edit folder flow и footer из `Target Folder` на экране `Move Source`, чтобы placement UI не показывал неинтерактивные пояснения; переименовать `Move Sources` в `Move Source`, потому что flow перемещает один выбранный источник;
- [x] `Feed Save Completion Stays On Sources`: изменить app-level completion после создания или редактирования источника: после успешного `Add Feed` / `Save Changes` закрывать `SourceManagementScreen`, обновлять список `Sources`, но оставаться на экране источников без автоматического перехода на экран статей сохранённого feed;
- [x] `Add Feed Nested Folder State Restoration`: сохранить состояние `add feed` flow при переходе в `Create New Folder` из уже полученного `Preview Feed`: при возврате назад пользователь должен видеть тот же URL, preview metadata, выбранную папку и доступный финальный `Add Feed`, а не сброшенную форму;
- [x] `Feed Save Dismisses Before Initial Refresh`: изменить completion после `Add Feed` / `Save Changes`, чтобы `SourceManagementScreen` закрывался сразу после успешного сохранения источника и reload `Sources`, а initial refresh запускался асинхронно и обновлял sidebar / article state после завершения без удержания пользователя на add/edit feed screen;
- [x] `Empty Folders Sidebar Visibility`: показывать на экране `Sources` все сохранённые папки, включая пустые, и сохранять для них context menu / long-press действия `Edit...` и `Delete`, чтобы папку можно было переименовать или удалить даже до добавления источников;
- [x] `Folder Name Case-Insensitive Uniqueness`: усилить invariant имени папки на уровне validation и service/repository boundary: запретить создание и переименование папок, которые отличаются только регистром или пробелами вокруг имени, например `Tech` и `TeCh`;
- [x] `Preview Feed System Activity Indicator`: заменить единственный кастомный loading indicator в `Preview Feed` на `UIActivityIndicatorView` через локальный `UIViewRepresentable`, чтобы проблемный add/edit feed preview-flow не зависел от lifecycle edge cases `ProgressView`, но визуально оставался согласованным с системными `ProgressView` и `.refreshable` в остальном приложении;
- [x] `Feed Display Name Override`: добавить пользовательское имя источника поверх title из feed XML, чтобы source title можно было задать вручную при добавлении и редактировании источника; refresh должен обновлять техническую metadata из XML, но не перетирать выбранное пользователем display name и связанные article/sidebar projections;
- [x] `Reader Adjacent Article Controls`: стабилизировать переход к следующей / предыдущей статье через явные controls в нижнем toolbar `ReaderView`: использовать текущий `articleNavigationContextIDs`, не выбирать повторно текущую статью, обновлять `articleScreenReloadID` при любой смене selected article, пересоздавать `ReaderView` для нового `articleID` и расположить controls в порядке `Unread`, `Star`, `Next`, `Previous`, `Safari`;
- [x] `Reader Swipe Boundary Navigation`: вернуть swipe / overscroll переход к следующей и предыдущей статье поверх стабилизированного adjacent article flow: обычный вертикальный жест сначала прокручивает текущую статью, затем при pull за верхнюю / нижнюю scroll boundary показывает debug-style индикатор `minus -> chevron`, а переключение происходит только после отпускания пальца; верхний overscroll считается от `contentOffset` / `contentInsetTop`, нижний — отдельно для scrollable и non-scrollable content, чтобы короткие статьи без прокрутки и длинные статьи использовали предсказуемую boundary-механику и тот же app-level adjacent article path, что и toolbar controls;
- [x] `Reader Adjacent Article Transition Animation`: добавить анимацию при переходе к следующей или предыдущей статье из `ReaderView`, чтобы смена article ощущалась как направленная прокрутка / slide по текущему adjacent article direction, а не как резкая замена контента;
- [x] `Reader Boundary Navigation Haptics`: добавить тактильный сигнал в момент, когда overscroll-индикатор достигает ready-state и `minus` заменяется на `chevron.up` / `chevron.down`; haptic не должен срабатывать при первом появлении `minus` и не должен повторяться во время одного pull-жеста;
- [x] `Reader Chrome Translucent Scroll Underlay`: сделать так, чтобы на экране статьи контент при прокрутке был виден сквозь верхний navigation bar и нижний toolbar / button area, а не обрезался жёстко текущими safe-area границами;
- [x] `Reader Adjacent Navigation Controls Setting`: добавить настройку для показа controls перехода между соседними статьями в `ReaderView`: только swipes без toolbar controls или swipes вместе с toolbar controls; по умолчанию оставить текущий режим со swipes и toolbar controls;
- [x] `Settings Close And Apply Controls`: обновить chrome окна настроек: заменить `Done` на compact close-control с крестиком, добавить отдельную кнопку с галочкой для применения изменений настроек и заменить иконку входа в settings с `ellipsis` на `gearshape`;
- [x] `Sources Folder Visibility By Filter`: изменить видимость папок на экране `Sources`: при фильтре `All Items` показывать все папки, а при фильтрах `Unread` и `Starred` скрывать пустые для текущего фильтра папки;
- [x] `Add Feed And Reading Defaults Polish`: заменить placeholder адреса источника с точного `example.com/feed.xml` на более общий `example.com`, чтобы поле ввода подсказывало возможность добавить сайт без знания RSS endpoint; при переходе на экран источника показывать пользовательский display name в заголовке, если он задан; сделать сортировку статей по умолчанию `Oldest First`;
- [x] `Reader Article Image Cache`: настроить общий `URLCache` приложения для изображений статей, чтобы `AsyncImage` мог переиспользовать уже загруженные картинки при переключении между статьями, если URL и HTTP cache policy позволяют использовать локальный результат;
- [x] `Reader Article Image Memory Cache`: заменить прямой `AsyncImage` в `ReaderView` на `CachedArticleImageView`, который хранит декодированные изображения в memory cache по `URL` и синхронно показывает cache hit при переключении между статьями;
- [x] `Reader Article Image Disk Cache`: добавить bounded disk cache для bytes изображений статей, чтобы кеш переживал перезапуск приложения, экономил трафик и очищался автоматически по LRU при превышении фиксированного лимита;
- [x] `Reader Article Image Cache Reset Control`: добавить в самый низ `Settings Screen` кнопку ручной очистки локального кеша изображений статей, чтобы пользователь мог удалить сохранённые на устройстве cache files без переустановки приложения;
- [x] `Article Archive Metadata`: изменить модель `Article` без legacy migration support: добавить `archivedAt`, перевести feed reconciliation с немедленного скрытия отсутствующих в RSS статей на пометку архивного состояния, а при повторном появлении статьи в RSS сбрасывать архивный статус;
- [x] `Article Archive Visibility`: обновить article query/read-model слой так, чтобы списки и reader могли показывать current и архивные статьи в рамках retention semantics, а `isDeletedAtSource`/`archivedAt` больше не скрывал статью сам по себе;
- [x] `Article Retention Setting`: добавить настройку срока хранения архивных статей с вариантами `None`, `2 Days`, `1 Week`, `2 Weeks`, `1 Month`, сохранить её в `AppSettings` и показать в `Settings Screen`;
- [x] `Article Retention Cleanup`: добавить cleanup архивных статей по retention policy после manual/background refresh и при сохранении настройки; retention считать от `archivedAt`, не от `publishedAt`, а starred статьи не удалять автоматически, пока звезда установлена;
- [x] `Article Sync Scope Migration`: без поддержки старой модели данных перевести article payload в sync-backed scope или ввести sync-backed article payload model после проверки SwiftData/CloudKit constraints, чтобы архивные, unread и starred статьи материализовались на втором устройстве вместе с `ArticleState`, но изображения оставались локальным cache;
- [x] `Archived Articles Manual Purge`: добавить явную кнопку очистки архивных статей, которая удаляет archived articles без звезды независимо от `Article Retention Setting`, чтобы пользователь мог вручную сократить объём article payload, синхронизируемый через iCloud между устройствами;
- [x] `Persistence Bounded Growth Cleanup`: ограничить рост служебных persistence-данных: удалять `FeedFetchLog` старше 1 недели и удалять `ArticleState`, для которых больше нет соответствующего `Article`, если state не starred; starred `ArticleState` сохранять как пользовательское сохранение, а при повторном появлении очищенной статьи в feed считать её новой непрочитанной статьёй;
- [x] `Article Sync Legacy Cleanup`: после переноса article payload в sync-backed scope удалить оставшийся legacy-код и устаревшие compatibility paths, если они больше не нужны для текущей модели без legacy migration support;
- [x] `Reader Adjacent Navigation Mode Options`: расширить настройку навигации между соседними статьями до вариантов `Buttons`, `Swipes`, `Both`, чтобы пользователь мог выбрать только кнопки, только свайпы или оба способа, а selected value оставался коротким и помещался справа в строке настройки;
- [x] `Article Section Header Blur`: подобрать и реализовать blur/visual effect под sticky заголовками секций на `Articles Screen`, чтобы даты группировки меньше мешали при прокрутке списка статей;
- [x] `Unread App Icon Badge`: добавить badge на иконку приложения по количеству непрочитанных статей и обновлять его после refresh, изменения read/unread state и массовых действий;
- [x] `Unread App Icon Badge Setting`: добавить настройку показа количества непрочитанных статей на иконке приложения: по умолчанию badge выключен, а при включении в `Settings Screen` пользователь получает системный запрос разрешения уведомлений только для badge; в тексте настройки явно предупредить, что приложение не выполняет рассылку уведомлений, а разрешение нужно системе только для отображения badge на иконке;
- [x] `Preview Feed Button Background`: убрать prominent capsule-фон у кнопки `Preview Feed` в add/edit feed flow, чтобы в тёмной теме действие не лежало на лишней серой плашке и визуально оставалось частью формы `SourceManagementScreen`;
- [x] `Article Opening Settings Semantics`: убрать неработающий сценарий `Reader Mode` / full-text extraction из `Open Articles` и связанный extension point из article renderer state; оставить в настройке только реально различающиеся варианты открытия статьи — app-owned reader и `In-App Browser`, переименовав app-owned reader option так, чтобы было понятно, что она показывает контент из RSS / Atom внутри приложения; переименовать и при необходимости поднять выше настройку `Original Article Link`, чтобы было ясно, что она управляет кнопкой открытия оригинальной web-страницы из `ReaderView`, а не способом открытия статьи из списка; обновить footer секции `Reading` под новую семантику настроек;
- [x] `Reader HTML Payload Normalizer`: добавить отдельный normalizer для RSS / Atom body payloads, который распознаёт plain text, HTML и escaped HTML, декодирует HTML entities включая numeric entities, нормализует whitespace / paragraph boundaries и передаёт `ArticleScreenContentRenderer` уже определённый тип body content, чтобы literal `<p>` / `<a href=...>` fragments не попадали в reader как обычный текст;
...
- [x] `Sources Last Refresh Subtitle`: показывать время последнего обновления только на экране источников: сохранять app-level `lastSourcesRefreshAt` после ручного refresh всех sources и после executed automatic background refresh, не менять timestamp при простом открытии / reload / single-source refresh, форматировать как `Today at HH:mm`, `Yesterday at HH:mm` или `Sunday, 24 May 2026`; на экране статей источника вместо времени показывать unread count и `No Unread Items` при нуле;
- [x] `Unread Filter Session Visibility`: изменить поведение `Unread` внутри открытого экрана статей: статья, автоматически отмеченная прочитанной при открытии reader, должна оставаться в текущем списке как прочитанная до выхода пользователя с экрана статей / текущей source selection и повторного входа; повторная загрузка selection должна заново применять `ArticleListFilter.unread`, чтобы уже прочитанные статьи исчезали только при новом входе в список;
- [x] `Unread Refresh Snapshot Stabilization`: стабилизировать ручной refresh для `Unread` после чтения статей: обычные reload экрана удерживают session-read статьи в текущем snapshot и показывают их как прочитанные, manual refresh во время `.refreshable` сохраняет структуру `List`, после завершения refresh отложенно сбрасывает session-read overlay, заново применяет настоящий `ArticleListFilter.unread`, а затем пересоздаёт `List` после анимации удаления строк, чтобы системный индикатор pull-to-refresh не схлопывался и прочитанные статьи исчезали плавно;
- [x] `Article List Refresh Start Haptic`: добавить тактильный сигнал в момент фактического старта ручного обновления списка статей: haptic должен срабатывать внутри refresh action перед запуском refresh pipeline, а не при первом появлении pull-to-refresh индикатора, чтобы пользователь чувствовал именно начало обновления;
- [x] `Article List Return Refresh Stability`: стабилизировать системный pull-to-refresh после возврата из reader к списку статей: пересоздавать внутренний SwiftUI `List` при переходе от выбранной статьи обратно к списку, чтобы refresh control не оставался в нестабильном состоянии после открытия уже прочитанной статьи в `All Items` / обычных списках и следующий manual refresh не схлопывал индикатор сразу после старта;
- [x] `Reader Header Metadata Spacing`: уплотнить header layout в `ReaderView`, чтобы `author` и `feedTitle` визуально относились к заголовку статьи: убрать лишний общий `VStack` spacing между title / author / source, но сохранить читаемый отступ перед body blocks;
- [x] `Source Icon Metadata Recovery`: разобраться с pipeline картинок источников от feed metadata до sidebar UI: проверить извлечение `icon` / `logo` / RSS image / fallback favicon, сохранение `Feed.iconURL`, загрузку через `SourceIconCacheService` и отображение `FeedSidebarItem.iconURL`, чтобы `newspaper` оставался только fallback, а не постоянным состоянием для всех источников;
- [x] `Source Icon Disk Cache`: добавить отдельный bounded disk cache для иконок источников поверх `SourceIconCacheService`, чтобы найденные favicon / apple-touch-icon / RSS image bytes переиспользовались после перезапуска приложения без повторного network discovery; держать cache lifecycle отдельно от `ArticleImageDiskCache`, но в `Settings Screen` добавить отдельную кнопку очистки source icon cache рядом с существующей очисткой кеша картинок статей;
- [x] `Source Icon Placeholder Background`: убрать видимый задний фон у fallback-символа `newspaper` в `SourceIconView`, чтобы placeholder отсутствующей иконки источника выглядел как обычный системный символ без серой плашки и не конкурировал с настоящими source icons;
- [x] `Settings Apply Button Initial State`: исправить начальное состояние кнопки применения настроек в `Settings Screen`, чтобы confirmation action оставался disabled сразу после открытия экрана и включался только после реального изменения draft-настроек относительно сохранённого `AppSettingsSnapshot`;
- [x] `Reader Author Display Normalization`: нормализовать отображение автора в `ReaderView`: если feed отдаёт автора вместе с email, показывать только имя и фамилию, а технический email вроде `<name@example.com>` не выводить в reader metadata;
- [x] `Reader Title Original Article Link`: сделать заголовок статьи в `ReaderView` интерактивной ссылкой на оригинальную статью: tap по title должен использовать ту же `ArticleSourceLinkOpeningPolicy` и тот же route flow, что и кнопка `Open Original Article`;
- [x] `Reader Missing Body Placeholder Copy`: переписать placeholder для статей без body content, чтобы текст говорил, что для данной статьи в полученном feed нет содержимого, а не утверждал, что весь feed не предоставляет full article content;
- [x] `Article List Scroll Position Restoration`: сохранять scroll position списка статей при переходе в reader и возврате назад, чтобы после открытия статьи из середины / конца списка пользователь возвращался примерно к той же высоте и той же области списка, а не попадал обратно наверх;
- [x] `Article List Session Model`: ввести screen-level `ArticleListSession` / `ArticleListEntry` для `Articles Screen`, чтобы текущий список статей стал материализованным display snapshot с явным `context`, а не прямым отражением последнего результата `ArticleQueryService`;
- [x] `Article List Membership Policy`: описать и покрыть тестами промежуточные статусы строк списка (`matchesCurrentQuery`, `retainedAfterRead`, `retainedAfterRefresh`), чтобы статья могла быть прочитанной в persistence, но оставаться видимой в текущей unread/list session до смены контекста;
- [x] `Article List Session Merge`: перенести refresh/load merge-policy в `ArticlesScreenController` или отдельный reducer: свежий query snapshot должен обновлять существующие строки, добавлять новые статьи и сохранять retained entries текущей session без очистки списка и без пересоздания `List`;
- [x] `Article Read-On-Open Session Update`: заменить app-level удержание `articleListSessionReadArticleIDs` на screen-level update текущей `ArticleListSession`: `ReaderView` / `ArticleScreenController` должны сообщать о `markAsReadOnOpen`, а список должен обновлять строку как read без удаления из текущего display snapshot;
- [x] `Article List Refresh Without List Identity Reset`: убрать зависимость стабильности refresh от `articleListIdentity`: manual refresh должен работать поверх существующего `List`, не сбрасывать его identity, не очищать display snapshot во время `.refreshable` и применять результат одним commit после завершения refresh pipeline;
- [x] `Article List Native Refresh Contract`: зафиксировать state contract для системного `.refreshable`: refresh action живёт до завершения `FeedRefreshService`, `refreshState` не переводит уже загруженный список в primary loading, а refresh banner / haptic не конкурируют с системным индикатором;
- [x] `Article List Scroll Continuity Cleanup`: удалить ручное восстановление позиции через `ScrollViewReader.scrollTo` и задержки после пересоздания списка; возврат из reader должен опираться на стабильные row ids, неизменный `List` identity и сохранённый display snapshot текущей `ArticleListSession`;
- [x] `Article List Session Context Reset`: закрепить lifecycle текущей `ArticleListSession`: смена `sourceSelection` или `SourcesFilter` создаёт новый materialized snapshot по актуальному `ArticleListFilter`, возврат из reader внутри того же context сохраняет session snapshot и retained read entries, а manual refresh завершает retained read overlay и применяет свежий query snapshot без пересоздания `List`;
- [x] `Refresh Indicator Contract`: определить единый contract refresh-индикатора для приложения: выбрать, остаётся ли UIKit/system activity indicator для коротких form/search сценариев или вводится общий кастомный символ обновления, например вращающееся незамкнутое кольцо, и зафиксировать states `idle` / `pulling` / `ready` / `refreshing`;
- [x] `Article List Custom Refresh State`: ввести screen-level state для кастомного refresh на `Articles Screen`, отделив состояние pull gesture и видимость индикатора от `ArticleListSession`, `refreshState` загрузки данных и системного `.refreshable`;
- [x] `Article List Pull Detector Spike`: заменить `.refreshable` на `Articles Screen` диагностическим custom pull detector без пересоздания `List`: проверить вариант поверх текущего `List` и, если он не даёт стабильной геометрии, отдельно оценить переход списка статей на `ScrollView` / `LazyVStack`;
- [x] `Article List Custom Refresh Wiring`: связать custom refresh control с существующим `refreshCurrentSelection` flow: action должен запускать `FeedRefreshService`, затем выполнять `loadArticles(retainsSessionReadArticles: false, retainedSessionReadMembershipStatus: .retainedAfterRefresh)`, показывать ошибку через текущий refresh banner и не трогать `articleListIdentity`;
- [x] `Article List Refresh Return Validation`: покрыть и вручную проверить критичный сценарий `Articles -> Reader -> Articles -> pull to refresh`: индикатор должен оставаться видимым до окончания refresh, scroll position не должна сбрасываться, а прочитанная retained статья должна исчезать только после manual refresh как результат свежего query snapshot;
- [x] `Refresh Indicator Shared Symbol`: если для `Articles Screen` вводится кастомный символ обновления, вынести его в общий UI-компонент и перевести на него все app-owned refresh-индикаторы, чтобы `Articles Screen`, `Sources Screen` и source discovery / preview flow не использовали разные визуальные метафоры для одного действия;
- [x] `Sources Refresh Indicator Alignment`: проверить refresh-индикаторы на экране источников: manual refresh источников, sidebar refresh status и связанные loading states должны либо использовать общий custom refresh symbol, либо явно остаться на системном UIKit-style indicator, если это часть form/search UX;
- [x] `Refresh Start Haptic Coverage`: проверить, не потерян ли тактильный отклик при запуске ручного обновления в flows, где уже используется кастомный refresh indicator; haptic должен срабатывать по тому же смысловому моменту, что и при переходе к следующей / предыдущей статье, то есть в момент фактического старта действия, а не просто при появлении визуального состояния;
- [x] `Sort Unread Articles Semantics`: переосмыслить настройку `Sort Articles` как `Sort Unread Articles`, чтобы базовая сортировка списков по умолчанию показывала самые свежие статьи сверху, а для `Unread` filter можно было отдельно выбрать режим со старыми непрочитанными статьями вверху;
- [x] `Sort Unread Articles Contract Cleanup`: без legacy migration support переименовать persisted/settings contract сортировки из общего `sortMode` / `articleSortMode` в unread-specific contract, чтобы модель настроек, screen state и UI identifiers не сохраняли старую семантику общей сортировки статей;
- [x] `Shared Loading Indicator Tint And Reuse`: расширить `AppRefreshIndicator` tint / foreground-style параметром и использовать общий custom refresh symbol для загрузки картинок в `CachedArticleImageView`; на экране проверки источника `Preview Feed` показывать тот же символ во вторичном цвете рядом с серым label, чтобы loading states не расходились визуально и не сохраняли системный `ProgressView` там, где уже есть app-owned refresh symbol;
- [x] `Article Title Text Normalization`: нормализовать заголовок статьи до сохранения / показа в списке и reader: декодировать HTML entities вроде `&amp;#132;` / `&amp;#147;`, убрать невидимые управляющие символы из RSS / CDATA payload и сохранить читаемый display title без технических артефактов из feed XML;
- [x] `Source Icon Appears On Add`: при сохранении нового или отредактированного источника сразу запускать загрузку / discovery иконки для этого feed, чтобы sidebar показывал source icon после добавления источника без ожидания ручного обновления.

#### Post-Interface Cleanup
- [x] `App Dependencies Composition Split`: разделить `AppDependencies` на явные composition-фабрики и runtime action surface: оставить в `AppDependencies` хранение собранных зависимостей, а bootstrap SwiftData / CloudKit, cache directories, background refresh wiring и app-level UI actions вынести в небольшие файлы / extension-группы с понятными границами ответственности;
- [x] `App Action Routing Cleanup`: отделить app-level navigation / Safari / source-management actions от infrastructure composition: `showInbox`, `selectArticle`, `openArticleInSafari`, source-management loaders и refresh entry points должны выглядеть как app action layer поверх `AppState` и services, а не как часть сборки dependency graph;
- [x] `Reader Opening Contract Rename`: без legacy migration support переименовать `ReaderMode` / `defaultReaderMode` в контракт открытия статьи, который прямо описывает выбор между app-owned feed reader и `SFSafariViewController`; убрать оставшуюся browser/reader-mode терминологию, которая была уместна до удаления `WKWebView` и full-text reader extension point;
- [x] `Unread Sort Contract Tightening`: сузить naming сортировки до unread-specific semantics на границе settings / screen state / query selection: убрать остаточную общую терминологию `sortMode` там, где она фактически означает настройку `Sort Unread Articles`, но не ломать repository-level сортировку, если она остаётся универсальным query parameter;
- [x] `Source Management State Partition`: разрезать `SourceManagementScreenState` на отдельные state/reducer файлы для `Add Feed`, `Move Source` и `Create Folder`, оставив root state только координатором presentation destination и derived view state; сохранить текущий behavior, но сделать каждый flow тестируемым и review-friendly отдельно;
- [x] `Source Management View Split`: разделить крупный `SourceManagementScreenView` на root navigation container, destination factory и отдельные view-файлы для add/edit feed, move source и create/edit folder, чтобы UI-слой не смешивал navigation plumbing, bindings и конкретную разметку форм;
- [x] `Reader Content Rendering Pipeline Split`: разнести `ArticleScreenContentRenderer` на доменные части: body block models, HTML block tokenizer / parser, inline text renderer, media URL resolver и final body assembly; renderer должен остаться thin orchestration layer между `ArticleScreenBodyPayloadNormalizer` и UI presentation blocks;
- [x] `Feed Parsing Module Split`: разделить `FeedParserService` на XML document builder, feed kind detection, RSS parser, Atom parser и parse pipeline diagnostics, чтобы будущие tests для parsing / normalizer не зависели от одного большого файла с несколькими уровнями ответственности;
- [x] `Refresh Pipeline Boundary Cleanup`: проверить `FeedRefreshService` после накопленных archive / retention / diagnostics изменений и выделить из него отдельные шаги conditional request, not-modified handling, fetched response reconciliation, diagnostics/log persistence и batch orchestration, не меняя публичный `FeedRefreshCoordinating` contract без необходимости;
...
- [x] `App Dependencies Test File Physical Split`: после логического split по `@Suite` физически разнести `AppDependenciesTests` на файлы по composition areas и вынести локальные doubles / helpers в focused test support, чтобы targeted test runs не требовали навигации по одному большому файлу;
- [x] `Reader View Interaction Split`: разрезать `ReaderView` на root container, article chrome/actions, adjacent navigation gestures / overscroll indicators, body block presentation и cached image loading views; оставить root view координатором `ArticleScreenState`, `ArticleScreenActionHandlers` и renderer output;
- [x] `Sidebar View Boundary Split`: разделить `SidebarView` на navigation container, toolbar/actions, source filter menu, rows/sections и source icon loading view, чтобы sidebar UI не смешивал routing, custom refresh gesture и favicon cache/network fallback;
- [x] `Settings Presentation Boundary Split`: разнести `SettingsScreenPresentationModels` и `SettingsScreenController` на section builders / action handlers по areas: appearance, reading, article list, updates & sync, notifications и storage; root controller должен координировать loading/apply flow, а не хранить всю presentation policy;
...
- [x] `OPML Import Persistence`: применить accepted import preview через существующие source-management invariants: создать недостающие folders, создать только valid non-duplicate feeds, пропустить duplicates/invalid entries, вернуть import result с created/skipped counts и покрыть persistence boundary tests без прямого обхода repository rules из UI;
- [x] `OPML Export Service`: собрать OPML 2.0 XML из текущих active `Feed` / `Folder` summaries без SwiftUI: экспортировать flat folders как one-level folder outlines, ungrouped feeds на root level, `xmlUrl`, `htmlUrl`, `title` / `text`, стабильный порядок и escaping; покрыть deterministic output tests;
...
#### Article Search And Reading Polish Foundation
- [x] `Article Search Scope Contract`: зафиксировать searchable fields для статей (`title`, `summary`, `contentText` / plain-text fallback из `contentHTML`, `author`, `feedTitle`), selection scope для `Inbox` / `Unread` / `Starred` / `Folder` / `Feed` и явную policy для `hidden` и `archivedAt` как текущего deleted-at-source marker; отдельно решить, нужен ли публичный alias/DTO flag `isDeletedAtSource` или roadmap должен окончательно говорить через `archivedAt`; поиск должен наследовать текущий `SidebarSelection` и `SidebarArticleFilter`, не показывать hidden вне hidden-specific flows, не смешивать archived с обычными списками до отдельного archive UI и не возвращать удалённые у источника статьи, если они уже исключены текущим list scope; покрыть scope contract unit tests на уровне query/presentation helpers;
- [x] `Article Search Query Layer`: заменить текущий local filtering в `ArticlesScreenState.derivedViewState(searchText:)` на query/read-model API в `ArticleQueryService`: ввести search request/criteria с `SidebarSelection`, `SidebarArticleFilter`, normalized query, sort mode и limit/empty-query behavior; оставить `View` только владельцем текста поиска и reload trigger, а matching/ranking/filtering держать в query layer; покрыть `ArticleQueryServiceTests`;
- [x] `Article Search UI Contract`: подключить `Articles Screen` к search query contract через `ArticlesScreenController`: поиск должен обновлять `ArticleListSession`, `visibleArticleIDs`, `articleNavigationContextIDs`, subtitle и `Mark all as read` только по результатам query; разделить empty states `нет статей в selection` и `нет результатов поиска`, сохранить loading/error UX и стабилизацию selection при смене search text; покрыть tests для `ArticlesScreenState` / `ArticlesScreenController`;
- [x] `Reader Adjacent Article Transition State Stabilization`: стабилизировать переходы к предыдущей/следующей статье перед отдельной визуальной полировкой overscroll: сделать `ReaderAdjacentArticleNavigationDirection` `Equatable`, ограничить preserved stale content через `ArticleScreenState.derivedViewState(... preservedStaleArticleID:)`, заменить одиночный `adjacentArticleTransitionDirection` на `AdjacentArticleTransitionContext` с `sourceArticleID`, `targetArticleID` и generation token, чтобы async load/reset в `ReaderView` применялись только к актуальному переходу и не сбрасывали transition state для уже сменившейся статьи;
- [x] `Unread Archived Retention Alignment`: выровнять unread-счётчики и списки статей вокруг retained archived articles: sidebar, folder/feed rows, smart row, app badge и `Articles Screen` должны учитывать все сохранённые на устройстве unread-статьи, включая `archivedAt != nil`, пока они физически не удалены retention cleanup после refresh; `hidden` / `read` остаются обычными фильтрами; `ArticleQueryService` и `ArticleSearchScope` не должны скрывать archived-статьи до удаления; после refresh/cleanup нужно обновлять sidebar/article reload triggers, чтобы counters и visible list пересчитывались вместе; покрыть repository, query/search, sidebar, articles-screen, retention и app-flow tests;
- [x] `Feed Unsubscribe Confirmation`: добавить предупреждение перед отпиской от ленты из sidebar context menu: destructive action должен сначала создать pending confirmation state с названием ленты, а `AppActionRouter.unsubscribeFeed` вызываться только после подтверждения; отмена не меняет selection, sidebar/article reload IDs и badge state; покрыть sidebar/app-flow tests;
- [x] `Folder Delete Confirmation`: добавить предупреждение перед удалением папки из sidebar context menu: destructive action должен сначала создать pending confirmation state с названием папки и сообщением, что удаление папки не удаляет находящиеся в ней ленты; `AppActionRouter.deleteFolder` должен вызываться только после подтверждения; отмена не меняет selection, sidebar/article reload IDs и badge state; покрыть sidebar/app-flow tests;
- [x] `Feed URL Edit Save State Machine Fix`: исправить зависание проверки при изменении URL существующей ленты: в `FeedManagementAddFeedState` / `FeedManagementScreenController+AddFeed` зафиксировать state machine `URL changed -> preview loaded for normalized URL -> update command uses that preview -> save completes`, включая случай, когда requested URL и resolved feed URL отличаются; после успешного save закрывать feed-management flow без повторного бесконечного preview; покрыть `FeedManagementAddFeedStateTests` и controller tests; это не является полноценной заменой источника, потому старые статьи, icon metadata и refresh validators остаются отдельным долгом;
- [x] `Feed Edit URL Flow Rollback`: отказаться от превращения одной ленты в другую через edit-flow: откатить код и тесты, добавленные для `Feed Source Replacement Persistence Reset`, включая `FeedRepository.replaceSource`, `FeedSourceReplacementUpdate`, feed-management зависимость от `ArticleStateRepository`, очистку статей / `ArticleState` при смене URL и regression test `t-j.ru -> nplus1.ru`; убрать из edit-flow возможность менять feed URL и использовать preview новой ленты, оставив для существующей ленты только rename display title через обычный `FeedManagementUpdateFeedCommand`; обновить `AgentDocs/CURRENT_ARCHITECTURE.md`, чтобы текущий contract снова говорил: смена адреса ленты выполняется через добавление новой ленты и удаление старой, а не через source replacement;
- [x] `Feed Rename Sheet`: переименовать context menu action долгого тапа по ленте с `Изменить...` на `Переименовать...`; заменить экран `Изменить ленту` на упрощённый `Переименовать ленту`, где остаётся только блок `Отображаемое имя` с text field и текущим описанием `Задайте имя, которое будет отображаться в списке лент и статей`; убрать из rename destination заголовок/секцию `Feed URL`, поле нового адреса, preview action, preview/status блоки, folder placement и inline create-folder; сделать presentation компактнее для direct modal launch, чтобы rename UI занимал нижнюю часть экрана, а не вытягивался на всю высоту; покрыть `FeedManagementAddFeedStateTests` / screen-state / controller tests на rename-only save и отсутствие URL preview в edit mode;
- [x] `Folder Rename Copy`: переименовать context menu action долгого тапа по папке с `Изменить...` на `Переименовать...`; в существующем окне редактирования папки изменить только navigation title с `Изменить папку` на `Переименовать папку`, не меняя поля, summary, validation, save flow и список существующих папок; покрыть локализационные / launch-context expectations в `FeedManagementScreenControllerLaunchContextTests`;
- [x] `Move Feed Organization Layout`: в direct flow `Организовать...` для ленты изменить порядок секций экрана `Переместить ленту`: после заголовка `Организовать ленты` и подписи сначала показывать блок `Папка назначения`, затем блок выбора ленты; переименовать последний блок с `Выберите ленту` на `Выбранная лента`; сохранить текущую логику preselected feed, validation и submit; покрыть `FeedManagementMoveFeedStateTests` / screen-state tests на порядок presentation sections и новые localization strings;
- [x] Стабилизировать policy иконок лент: скачивать иконку при добавлении ленты и при ручном refresh только если видимая иконка отсутствует, не обновлять cached icon при rename или явной очистке cache, а `Feed.iconURL` при refresh менять только если он ещё не сохранён;
- [x] `Feed Icon Discovery Service`: вынести лёгкий discovery иконок из `FeedIconView` в service layer: быстро проверять `iconURL` из XML, common favicon paths и HTML `<link rel="icon">`, сохранять первый валидный raster icon в cache / `Feed.iconURL`, а при неудаче оставлять текущий placeholder без монограмм и внешних favicon providers;
- [x] `Feed Icon Rendering Policy`: добавить `FeedIconRenderingPolicy` для видимости иконок на светлой и тёмной теме: определять слабый контраст маленькой иконки относительно текущего фона и добавлять контрастную rounded-square подложку или stroke, не переводя все иконки в grayscale и не меняя исходные cached assets;
- [x] `Reader Open Source Swipe Contract`: реализовать свайп справа-налево при открытой статье как shortcut к `openSourceArticle`: жест должен уважать `ArticleSourceLinkOpeningPolicy` / `ArticleOpeningMode` и текущий `AppActionRouter` boundary, открывая `SafariBrowserView` или внешний браузер так же, как toolbar action; жест не должен конфликтовать с vertical previous/next overscroll и compact back swipe; покрыть navigation policy tests;
- [x] `Article Opening Mode Safari Direct Route`: исправить shell-level flow для настройки `Открытие статей = Встроенный браузер`: tap по статье должен сразу открывать `SafariBrowserView` без установки `Reader` как текущего detail route, а закрытие Safari должно возвращать прямо к `Articles Screen`; при настройке `Открытие статей = Reader` сохранить текущий сценарий `Articles -> Reader -> Safari -> Reader`; разделить app-level state/action contract для reader-owned Safari presentation и direct Safari presentation из списка, обновить `RootView` / `AppActionRouter+Navigation` / `ReadingNavigationState` и покрыть shell tests на оба режима, fallback при unsupported URL и dismiss behavior;
- [x] `Safari Back Swipe And Direct Open Reading State`: добавить semantic leading-edge back swipe на `Safari Browser`, который вызывает существующий dismissal route и возвращает к `Articles Screen` для direct Safari flow либо к `Article Screen` для reader-owned Safari flow; direct Safari dismissal должен быть interactive: экран Safari следует за пальцем, показывает лежащий под ним `Articles Screen`, возвращается на место при cancel и завершает переход при commit; при прямом открытии статьи из списка применять существующую `markAsReadOnOpen` policy через `ArticleStateService` и публиковать `ArticleReadOnOpenEvent`, чтобы unread-статья оставалась в текущей list-session до её завершения; покрыть navigation policy и shell routing/state tests;
- [x] `Sidebar Filter Transition Animation`: добавить анимацию изменения состава `Sidebar` при переключении `All Items` / `Unread` / `Starred`: `List` должен сохранять строки со стабильными identity, а новые ленты и папки вставлять, скрываемые — удалять, плавно перестраивая позиции и вертикальный размер scroll content; использовать локальную `.snappy`-анимацию, учитывать Reduce Motion и не пересоздавать весь список при смене фильтра; покрыть существующими sidebar filtering/state/selection и shell filter-switch tests;
- [x] `Starred Filter Session Visibility`: выровнять поведение `Starred` с текущим session contract `Unread` на `Articles Screen`: при снятии star сразу обновлять `isStarred` и starred subtitle, но сохранять статью в текущем `ArticleListSession` до manual refresh или завершения list-session; manual refresh и новый вход должны применять свежий `ArticleListFilter.starred` snapshot и удалять статью через существующее изменение `visibleArticleIDs`, используя ту же `.snappy`-анимацию списка, что и `Unread`; покрыть mutation reducer, state и controller session reload/refresh tests;

#### Runtime Input And Data Lifecycle Hardening
- [x] `Resource Budget Contract`: зафиксировать в одном app-level contract отдельные ограничения для feed XML, discovery HTML, feed icons, article images и OPML: максимальный compressed body size, допустимые MIME types, image pixel dimensions, XML element/depth/entry counts и ожидаемую ошибку для каждого превышения; отразить текущие retention/cache/log ownership rules в `AgentDocs/DATA_LIFECYCLE.md`;
- [x] `Bounded HTTP Response Loading`: расширить `HTTPRequest` request-specific лимитом тела и заменить безусловный `URLSession.data(for:)` на bounded response loading: отклонять заведомо большой `Content-Length`, прекращать streaming download после превышения фактического лимита, сохранять cancellation и маппить resource-limit failure в typed networking/feed errors; покрыть transport tests для отсутствующего/ложного `Content-Length`, chunked response, cancellation и каждого request profile;
- [x] `Feed And Discovery Payload Limits`: применить разные HTTP budgets к `FeedFetcher`, feed-management discovery и `FeedIconDiscoveryService`; проверять response MIME до дорогого decode, ограничить HTML parsing и число discovery candidates, а также семплировать parser anomaly/rejected-entry logging вместо записи сообщения на каждый элемент; покрыть oversized XML/HTML/icon и diagnostics truncation tests;
- [x] `XML Parser Structural Limits`: добавить feed/OPML parser policy для максимальной глубины XML, числа элементов и количества materialized entries, чтобы full-tree `FeedXMLDocument` не мог бесконтрольно расти даже внутри разрешённого byte limit; возвращать понятную typed parse failure и покрыть deep nesting / excessive entries fixtures;
- [x] `Bounded OPML Import`: до `Data(contentsOf:)` проверять размер выбранного security-scoped файла, читать только в пределах OPML budget и выполнять parse/preview planning вне `MainActor`, возвращая отдельный localized status для слишком большого файла; покрыть file-size boundary и cancellation tests;
- [x] `Bounded Article Image Loading`: заменить прямой `URLSession.shared.data(from:)` на отдельный image loader с byte/MIME/pixel limits, cancellation и decode downsampling под фактический display size; не помещать oversized/invalid payload в memory или disk cache; покрыть compressed-small/decoded-huge image, invalid MIME, cancellation и cache-bypass tests;
- [x] `Current Article Retention Contract`: принять и зафиксировать product rule для статей, которые feed продолжает возвращать: starred articles сохраняются без ограничения, а остальные current/archived articles должны иметь явный time/count budget; обновить settings copy так, чтобы выбранная policy однозначно описывала срок хранения всех XML-materialized статей, а не только исчезнувших из feed;
- [x] `Bounded Current Article Cleanup`: реализовать выбранный retention contract через repository predicates/batches с защитой starred state, корректной очисткой orphan `ArticleState`, sidebar/badge reload и CloudKit-compatible behavior; покрыть feed с постоянной многолетней историей, missing dates, current/archived/starred combinations и repeated cleanup idempotency.

#### Cache Ownership And Memory Boundaries
- [ ] `Single Article Image Disk Cache`: выбрать один disk-cache owner для article images: оставить custom LRU cache и загружать изображения через session без shared `URLCache` либо полностью перейти на bounded `URLCache`; удалить двойное хранение по 200 MB, сохранить Settings clear action и покрыть cold/warm/relaunch behavior;
- [ ] `Decoded Image Memory Accounting`: считать `NSCache` cost по decoded bitmap footprint/downsampled image, а не по compressed `Data.count`; убрать бесконечно растущие `storedURLs` / `cachedURLs` либо синхронизировать их через bounded index/eviction delegate; покрыть automatic eviction, `hasCachedData` и repeated unique-URL load tests;
- [ ] `Feed Icon Cache Boundary Cleanup`: сделать `FeedIconCacheService` чистым memory/disk cache boundary: удалить production-unused `imageData(for:)`, `HTTPClient` dependency и duplicate in-flight network orchestration, оставив сеть только в `FeedIconDiscoveryService`; обновить tests вокруг cache-only contract;
- [ ] `Legacy Cache Directory Cleanup`: отделить disposable cache migration от no-legacy persistence migration и один раз удалить старый `RSSReaderSourceIcons` после перехода на `RSSReaderFeedIcons`; сделать cleanup idempotent и покрыть наличие old/new/both cache directories;
- [ ] `Cache Capacity Regression Matrix`: добавить focused tests для single-entry-over-capacity, aggregate LRU eviction, disk write failure, memory pressure/eviction bookkeeping и Settings cleanup всех cache owners;
...
- [ ] `Off-Main Feed Parsing Worker`: вынести XML tree build, feed kind detection, normalization, deduplication и entry filtering из `MainActor` в отдельный Sendable worker/actor; на `MainActor` оставить только SwiftData transaction и app-state effects; покрыть cancellation и actor-boundary tests;
- [ ] `Batch Article Upsert`: заменить N+1 `fetchArticle(feedID:externalID:)` для каждого payload на один fetch существующих статей feed, dictionary по normalized identity и batch insert/update с одним save; сохранить deduplication и CloudKit-compatible uniqueness behavior; покрыть mixed insert/update/duplicate payload tests;
- [ ] `Single-Snapshot Feed Reconciliation`: объединить projection refresh, missing-entry reconciliation и batch upsert вокруг одного existing-articles snapshot, чтобы один refresh не перечитывал и не обходил весь feed несколько раз; сохранить archive/reactivation semantics и единый rollback boundary;
- [ ] `Refresh Cancellation Checkpoints`: добавить cancellation checkpoints между parse, icon discovery, reconciliation, batch upsert и save; не проглатывать `CancellationError` внутри discovery helpers и гарантировать cleanup `inFlightRefreshTasks` без partial persistence;
- [ ] `Large Feed Pipeline Tests`: добавить deterministic fixtures для большого feed, измерить число repository fetch/save операций, проверить отсутствие MainActor starvation и зафиксировать приемлемый upper bound времени/памяти без brittle benchmark assertions;
...
- [ ] `Batched Article State Overlay And Counts`: ограничить `ArticleState` fetch только identities текущей page/query, заменить full-table unread count materialization на count/grouped query strategy и проверить badge/sidebar consistency после cleanup/sync;
- [ ] `Search Debounce And Cancellation`: добавить 200-300 ms debounce для пользовательского ввода, generation/cancellation guard в `ArticlesScreenController` и убрать повторный full-scope probe для empty state; stale search result не должен заменять более новый query snapshot;
- [ ] `Searchable Text Normalization`: не очищать `contentHTML` регулярными выражениями на каждом keystroke: сформировать bounded normalized searchable text при materialization/update статьи либо использовать отдельный индексируемый read-model field с явной migration/rebuild policy;
- [ ] `Article Query Limit And Pagination`: применять `limit`/page boundary до materialization полного результата, добавить incremental list loading со stable IDs и сохранить `ArticleListSession`, selection, navigation context и mark-all-read semantics между pages;
...
- [ ] `Feed Management Router Decomposition`: разделить `AppActionRouter+FeedManagement` на presentation launch/completion, feed/folder destructive actions и initial-refresh scheduling, сохранив `AppActionRouter` как app-level facade и текущие confirmation/action boundaries;
- [ ] `Large Screen Composition Cleanup`: вынести OPML file I/O bridge из `SettingsScreenView`, а gesture/transition coordinators из `ArticleListView` и `ReaderView` в существующие screen-level helpers/modifiers без переноса business logic во `View`; сохранить preview и navigation tests;
- [ ] `Project Deployment Target Alignment`: выровнять app/test deployment targets и добавить minimum-supported-runtime validation, чтобы tests не требовали более новую iOS, чем приложение; проверить project settings для Debug/Release;
- [ ] `Strict Concurrency Migration Baseline`: включить staged strict-concurrency diagnostics в Swift 5 mode, устранить предупреждения и документировать обоснование каждого `@unchecked Sendable`, затем отдельным проверяемым шагом перевести target/tests на Swift 6 language mode;
- [ ] `Filesystem And Local Cleanup`: переименовать `AppDependencies+Environment.swift`, удалив zero-width characters, упростить redundant icon-rel condition, проверить template-only `ContentView` и провести dead-code/public-surface audit без массового unrelated refactor;
- [ ] `UI Smoke Test Target Foundation`: добавить минимальный UI test target для launch и основных маршрутов `Sidebar -> Articles -> Article -> Safari`, Settings presentation и базового iPad-ready navigation smoke без дублирования unit tests;
- [ ] `Deterministic Validation Script`: добавить versioned `Scripts/validate.sh` с режимами `fast` и `full`: project/string-catalog checks, simulator discovery, clean build, targeted/full tests, `/tmp` DerivedData и `pipefail` при использовании `xcbeautify`; документировать одинаковый вызов для человека и standalone Codex;
- [ ] `GitHub CI Baseline`: добавить минимальный macOS GitHub Actions workflow с зафиксированной доступной версией Xcode, clean build/full unit tests и сохранением `.xcresult` при failure; не включать signing/App Store actions до `Release Prep`;

#### Pre-iPad Feature Completion
- [ ] `Community Feed URL Discovery Foundation`: расширить `Feed Management` preview/add flow отдельным discovery-normalization слоем для community-based сайтов, где пользователь вводит человекочитаемый URL сообщества или темы, а приложение пробует найти настоящий RSS/Atom feed URL; первый контракт должен принимать `reddit.com/r/iosprogramming`, `https://www.reddit.com/r/iosprogramming/` и короткую форму `reddit.com/iosprogramming`, нормализовать их в canonical community URL и проверять feed-кандидат вроде `https://www.reddit.com/r/iosprogramming/.rss`; discovery должен жить до `FeedFetcher` / `FeedParserService`, не смешиваться с OAuth, не хранить логин/пароль и возвращать понятные preview/validation ошибки; покрыть unit tests на URL normalization, candidate generation, duplicate detection через normalized feed URL и fallback к обычному RSS discovery;
- [ ] `Community Feed Discovery Profiles`: после Reddit заложить расширяемый список discovery profiles для похожих международных сайтов с тематическими сообществами или topic pages, за которыми удобно следить как за лентами: Lemmy / Kbin communities (`/c/<community>`), Hacker News sections / users / item discussion feeds, Lobsters tags, Stack Exchange tags / questions feeds, Mastodon hashtags / public timelines там, где instance отдаёт RSS/Atom; каждый profile должен быть маленьким adapter-правилом `input URL -> canonical page URL -> feed candidate URLs`, без отдельного account API и без site-specific business logic внутри `FeedManagementScreenController`;
- [ ] `Reader Linkable Metadata`: сделать metadata в `ReaderView` интерактивной там, где есть URL: название ленты должно открывать canonical feed/site page через тот же app-level opening boundary, что и другие reader links, а имя автора должно открывать author profile URL, если feed/parser смог его извлечь; для Reddit сохранить отображение автора в исходной форме вроде `/u/Impossible_Step6452`, но связать его с `https://www.reddit.com/u/Impossible_Step6452`; если author URL отсутствует, metadata остаётся обычным текстом; потребуется расширить parser/read-model/presentation contract для `authorURL` и `feedSiteURL` без подмены `Feed` naming на legacy `Source`; покрыть renderer/query/presentation tests;
- [ ] `Reader Image Detail Viewer`: добавить на `Article Screen` tap по inline image и открыть app-owned image detail view поверх reader: экран должен показывать выбранное изображение в отдельном view, поддерживать pinch-to-zoom, pan, double-tap zoom reset/fit, share/save action и сохранение в Photos library с корректным permission/error UX; image detail route должен получать уже resolved image URL/cache payload из reader presentation, не обращаться напрямую к parser state и не ломать текущий `ArticleSourceLinkOpeningPolicy`; покрыть state/navigation tests и ручной checklist для remote image, cached image, failed download, denied Photos permission и больших изображений;
- [ ] `Reader Previous Article Overscroll Polish`: исправить прозрачный блок при перелистывании статьи вверх к предыдущей статье: проверить `ReaderArticleContentSurface`, `ReaderAdjacentArticleOverscrollIndicator`, `ArticleScreenNavigationState.adjacentArticleOverscrollState` и transition `.previous`, чтобы stale content / chrome underlay / safe-area background не оставляли прозрачную область во время overscroll и смены `articleID`; добавить focused test для navigation state;
- [ ] `Onboarding Screen Foundation`: добавить first-run onboarding screen с короткими разделами: как добавлять ленты, группировать ленты в папки, обновлять ленты, переключаться между статьями, пользоваться article/list swipes, где находятся настройки бейджей и кэшей; вынести copy в localization, добавить persisted completion flag в settings/app state и entry point из `Settings Screen`, чтобы onboarding можно было открыть повторно; покрыть state/persistence tests.
...
- [ ] скрытие статьи;
- [ ] `AppIntents Article Recall`: после выхода iOS 27 и обновлённой Siri исследовать и реализовать AppIntents для поиска недавно прочитанных статей по естественному запросу: intent должен искать только сохранённые и не удалённые статьи, уважать privacy/local data boundaries и открывать найденную статью в текущем reader flow;
