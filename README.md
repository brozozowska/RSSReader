...
- [ ] `Source Icon Metadata Recovery`: разобраться с pipeline картинок источников от feed metadata до sidebar UI: проверить извлечение `icon` / `logo` / RSS image / fallback favicon, сохранение `Feed.iconURL`, загрузку через `SourceIconCacheService` и отображение `FeedSidebarItem.iconURL`, чтобы `newspaper` оставался только fallback, а не постоянным состоянием для всех источников;
