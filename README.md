...
- [ ] `Article List Pull Detector Spike`: заменить `.refreshable` на `Articles Screen` диагностическим custom pull detector без пересоздания `List`: проверить вариант поверх текущего `List` и, если он не даёт стабильной геометрии, отдельно оценить переход списка статей на `ScrollView` / `LazyVStack`;
