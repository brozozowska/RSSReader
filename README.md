...
- [ ] устранить environment blocker `CKAccountStatusTemporarilyUnavailable` для simulator validation: нужно добиться состояния, в котором хотя бы один `iOS Simulator` под тем же `Apple ID` и с включённым `iCloud Drive` доходит до валидного `CloudKit` account/runtime path без `NSCocoaErrorDomain 134400`, иначе дальнейшая sync validation на двух endpoint'ах неинформативна;
- [ ] завершить bootstrap flow для cross-endpoint `useiCloudSync`: app bootstrap не должен зависеть только от локального `AppSettings.useiCloudSync`, а после устранения account/runtime blocker нужно подтвердить, что app-level bootstrap preference действительно позволяет второму endpoint войти в `CloudKit` path до получения remote `AppSettings`;
- [ ] перепроверить cross-endpoint bootstrap propagation через `NSUbiquitousKeyValueStore`: если после стабилизации account/runtime path второй simulator всё ещё читает только локальный `UserDefaults` и не получает bootstrap preference из iCloud KVS, нужно отдельно решить, это runtime bug приложения или ограничение simulator validation environment;
- [ ] провести simulator baseline verification для sync runtime: оба `iOS Simulator` должны быть под одним `Apple ID`, `Settings Screen` должен показывать ожидаемый runtime status, а изменение `useiCloudSync` должно подтверждаться как применяющееся только после relaunch / `ModelContainer` rebuild и ручного `Trigger iCloud Sync`, где это требуется simulator workflow;
- [ ] проверить clean bootstrap на втором `iOS Simulator`: чистый запуск под тем же `Apple ID` должен подтянуть уже существующие `Feed` / `Folder` / `AppSettings` / `ArticleState`, но не должен материализовать local-only `Article` без локального refresh path;
- [ ] проверить на связке `iOS Simulator + iOS Simulator` границы `CloudKitSyncScope` и `AppPersistenceModelPartition`: `Feed` / `Folder` / `AppSettings` / `ArticleState` должны реплицироваться между simulator endpoint'ами, а `Article` / `FeedFetchLog` должны оставаться только local-only cache/diagnostic storage;
- [ ] проверить `Feed` create/update/delete flow на связке `iOS Simulator + iOS Simulator`: добавление нового источника, изменение metadata и удаление feed на первом endpoint должны корректно приходить на второй endpoint без ручного пересоздания структуры;
- [ ] проверить `Folder` structure и folder assignment flow на связке `iOS Simulator + iOS Simulator`: создание, переименование и reorder папок, а также перенос feed между папками и возврат в ungrouped state должны реплицироваться между endpoint'ами консистентно с `SourceManagementService`;
- [ ] проверить sync для user-facing и service-level persisted preferences на связке `iOS Simulator + iOS Simulator`: настройки чтения, сортировки, theme, link-opening policies, `selectedSourcesFilterRawValue` и `refreshIntervalPreference` должны приходить на второй endpoint как persisted values без смешивания с отдельным runtime scheduling следующего эпика `Background Refresh`;
- [ ] отдельно проверить cross-endpoint semantics для `useiCloudSync` на связке `iOS Simulator + iOS Simulator`: изменение sync preference на одном endpoint должно быть явно подтверждено как желаемое поведение для второго endpoint, включая relaunch requirement, влияние на следующий app bootstrap и необходимость manual sync trigger в simulator environment;
- [ ] проверить sync для `ArticleState` happy path на связке `iOS Simulator + iOS Simulator`: read/starred/hidden state должен синхронизироваться между endpoint'ами без потери локальных действий и без рассинхронизации с article list/query layer;
- [ ] проверить и при необходимости скорректировать conflict resolution `ArticleState` по `updatedAt` для сценария `iOS Simulator + iOS Simulator` и CloudKit merge behavior, включая почти одновременные действия на двух endpoint'ах;
- [ ] проверить cross-endpoint manual refresh scenario на связке `iOS Simulator + iOS Simulator`: после refresh и чтения на первом endpoint второй endpoint должен после ручного refresh получить тот же набор local `Article`, а synced `ArticleState` должен сразу скрыть уже прочитанные материалы из `Unread`;
- [ ] проверить recovery scenario после временного отсутствия сети или delayed import на связке `iOS Simulator + iOS Simulator`: второй endpoint должен догонять source structure и `ArticleState` после восстановления sync, не теряя local-only article cache semantics и не требуя повторного добавления источников вручную;
