...
- [ ] `Bounded OPML Import`: до `Data(contentsOf:)` проверять размер выбранного security-scoped файла, читать только в пределах OPML budget и выполнять parse/preview planning вне `MainActor`, возвращая отдельный localized status для слишком большого файла; покрыть file-size boundary и cancellation tests;
