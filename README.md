...
- [ ] `Reader Open Source Swipe Contract`: реализовать свайп справа-налево при открытой статье как shortcut к `openSourceArticle`: жест должен уважать `ArticleSourceLinkOpeningPolicy` / `ArticleOpeningMode` и текущий `AppActionRouter` boundary, открывая `SafariBrowserView` или внешний браузер так же, как toolbar action; жест не должен конфликтовать с vertical previous/next overscroll и compact back swipe; покрыть navigation policy tests;
