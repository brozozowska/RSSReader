...
- [ ] `Settings Close And Apply Controls`: обновить chrome окна настроек: заменить `Done` на compact close-control с крестиком, добавить отдельную кнопку с галочкой для применения изменений настроек и заменить иконку входа в settings с `ellipsis` на `gearshape`;
