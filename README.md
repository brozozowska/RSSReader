...
- [ ] `Article List Scroll Position Restoration`: сохранять scroll position списка статей при переходе в reader и возврате назад, чтобы после открытия статьи из середины / конца списка пользователь возвращался примерно к той же высоте и той же области списка, а не попадал обратно наверх;
