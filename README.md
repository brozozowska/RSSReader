...
- [ ] `Settings Apply Button Initial State`: исправить начальное состояние кнопки применения настроек в `Settings Screen`, чтобы confirmation action оставался disabled сразу после открытия экрана и включался только после реального изменения draft-настроек относительно сохранённого `AppSettingsSnapshot`;
