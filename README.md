...
- [ ] `Article List Native Refresh Contract`: зафиксировать state contract для системного `.refreshable`: refresh action живёт до завершения `FeedRefreshService`, `refreshState` не переводит уже загруженный список в primary loading, а refresh banner / haptic не конкурируют с системным индикатором;
