...
- [ ] `Source Organize Context Action`: добавить для источника действие `Organize...` из long-press / context menu, которое открывает отдельный flow перемещения выбранного источника между папками по модели существующего `Move Sources`, но сразу с предвыбранным feed и без сетевой проверки `Preview Feed`;
- [ ] `Source Management Compact Confirmation Controls`: привести оставшиеся create/edit окна `SourceManagementScreen` к единому compact confirmation pattern: заменить длинные navigation-bar кнопки с текстом на compact action с галочкой, убрать лишнюю нагрузку с navigation bar, а на экране `Add Source` заменить `Done` на системный close-control с крестиком;
