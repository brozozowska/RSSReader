...
- [ ] отделить screen-specific input `Settings Screen` от общего `AppSettingsSnapshot`: сейчас presentation pipeline экрана напрямую потребляет весь service snapshot, включая поля, которые не являются собственным UI input экрана, поэтому полезно ввести отдельную screen-level data model / normalized input для `Settings Screen`;
