...
- [ ] `Source Management Compact Confirmation Controls`: привести create folder, edit folder и edit feed flows к тому же compact confirmation pattern, что и add feed: заменить длинные navigation-bar кнопки с текстом на compact action с галочкой, убрать лишнюю нагрузку с navigation bar, на экране `Add Source` заменить `Done` на системный close-control с крестиком, а в edit feed flow держать `Preview Feed` над секцией `Destination Folder`, чтобы повторная проверка URL была явно отделена от выбора размещения;
