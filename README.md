...
- [ ] проверить `Feed` create flow на связке `iOS Simulator + iOS Simulator`: добавление нового источника на первом endpoint должно появляться на втором без ручного пересоздания структуры;
- [ ] проверить `Feed` update/delete flow на связке `iOS Simulator + iOS Simulator`: изменение metadata источника и удаление feed на первом endpoint должны корректно приходить на второй endpoint;
- [ ] проверить `Folder` structure flow на связке `iOS Simulator + iOS Simulator`: создание, переименование и reorder папок должны переноситься между endpoint'ами без рассинхронизации sidebar hierarchy;
- [ ] проверить folder assignment flow на связке `iOS Simulator + iOS Simulator`: перенос feed между папками и возврат в ungrouped state должны реплицироваться между endpoint'ами консистентно с `SourceManagementService`;
- [ ] проверить sync для presentation/settings preferences на связке `iOS Simulator + iOS Simulator`: настройки чтения, сортировки, theme и link-opening policies должны консистентно приходить на второй endpoint и обновлять UI без ручной правки persistence;
- [ ] проверить sync для service-level persisted preferences на связке `iOS Simulator + iOS Simulator`: `selectedSourcesFilterRawValue` и `refreshIntervalPreference` должны приходить на второй endpoint как persisted values, не смешиваясь при этом с отдельным runtime scheduling следующего эпика `Background Refresh`;
