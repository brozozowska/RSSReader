...
- [ ] `Reader Header Metadata Spacing`: уплотнить header layout в `ReaderView`, чтобы `author` и `feedTitle` визуально относились к заголовку статьи: убрать лишний общий `VStack` spacing между title / author / source, но сохранить читаемый отступ перед body blocks;
