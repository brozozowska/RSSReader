...
- [ ] `Article List Membership Policy`: описать и покрыть тестами промежуточные статусы строк списка (`matchesCurrentQuery`, `retainedAfterRead`, `retainedAfterRefresh`), чтобы статья могла быть прочитанной в persistence, но оставаться видимой в текущей unread/list session до смены контекста;
