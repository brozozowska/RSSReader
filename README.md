...
- [ ] `Article List Session Merge`: перенести refresh/load merge-policy в `ArticlesScreenController` или отдельный reducer: свежий query snapshot должен обновлять существующие строки, добавлять новые статьи и сохранять retained entries текущей session без очистки списка и без пересоздания `List`;
