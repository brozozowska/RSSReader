...
- [ ] `Article List Refresh Return Validation`: покрыть и вручную проверить критичный сценарий `Articles -> Reader -> Articles -> pull to refresh`: индикатор должен оставаться видимым до окончания refresh, scroll position не должна сбрасываться, а прочитанная retained статья должна исчезать только после manual refresh как результат свежего query snapshot;
