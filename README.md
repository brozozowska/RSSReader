...
- [ ] `Article List Session Context Reset`: определить правила завершения session: смена `sourceSelection`, смена `SourcesFilter`, выход назад к `Sources`, search-сценарии и explicit reload должны либо сохранять текущий snapshot, либо создавать новый по `ArticleListFilter` по заранее описанной policy;
- [ ] `Article List Session Tests`: добавить unit tests для `ArticleListSession` / reducer: read-on-open в `Unread` не удаляет статью, refresh в `Unread` сохраняет retained read entries и добавляет новые unread, `All Items` не теряет scroll/selection context после reader return, смена filter/source создаёт новый snapshot без retained entries;
- [ ] `Article List Custom Refresh Fallback Spike`: если после перехода на `ArticleListSession` системный `.refreshable` всё ещё дёргает или преждевременно скрывает индикатор, провести отдельный spike кастомного pull-to-refresh для `Articles Screen` и сравнить цену перехода на `List` без `.refreshable` или `ScrollView` / `LazyVStack` с ручным refresh control;
