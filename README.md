...
- [ ] `App Action Routing Cleanup`: отделить app-level navigation / Safari / source-management actions от infrastructure composition: `showInbox`, `selectArticle`, `openArticleInSafari`, source-management loaders и refresh entry points должны выглядеть как app action layer поверх `AppState` и services, а не как часть сборки dependency graph;
