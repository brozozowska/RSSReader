...
- [ ] `Sort Unread Articles Semantics`: переосмыслить настройку `Sort Articles` как `Sort Unread Articles`, чтобы базовая сортировка списков по умолчанию показывала самые свежие статьи сверху, а для `Unread` filter можно было отдельно выбрать режим со старыми непрочитанными статьями вверху;
