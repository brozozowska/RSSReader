...
- [ ] `Reader Boundary Navigation Haptics`: добавить тактильный сигнал в момент, когда overscroll-индикатор достигает ready-state и `minus` заменяется на `chevron.up` / `chevron.down`; haptic не должен срабатывать при первом появлении `minus` и не должен повторяться во время одного pull-жеста;
- [ ] `Reader Chrome Translucent Scroll Underlay`: сделать так, чтобы на экране статьи контент при прокрутке был виден сквозь верхний navigation bar и нижний toolbar / button area, а не обрезался жёстко текущими safe-area границами;
