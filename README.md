...
- [ ] `Feed Rename Sheet`: переименовать context menu action долгого тапа по ленте с `Изменить...` на `Переименовать...`; заменить экран `Изменить ленту` на упрощённый `Переименовать ленту`, где остаётся только блок `Отображаемое имя` с text field и текущим описанием `Задайте имя, которое будет отображаться в списке лент и статей`; убрать из rename destination заголовок/секцию `Feed URL`, поле нового адреса, preview action, preview/status блоки, folder placement и inline create-folder; сделать presentation компактнее для direct modal launch, чтобы rename UI занимал нижнюю часть экрана, а не вытягивался на всю высоту; покрыть `FeedManagementAddFeedStateTests` / screen-state / controller tests на rename-only save и отсутствие URL preview в edit mode;
