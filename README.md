...
- [ ] `Article List Scroll Continuity Cleanup`: удалить ручное восстановление позиции через `ScrollViewReader.scrollTo` и задержки после пересоздания списка; возврат из reader должен опираться на стабильные row ids, неизменный `List` identity и сохранённый display snapshot текущей `ArticleListSession`;
