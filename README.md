...
- [ ] `Safari App Wiring`: переименовать `AppDependencies.openArticleInWebView` / `closePresentedArticleWebView` в Safari-oriented entry points и убедиться, что source article / body link opening проходят через новый app-level Safari flow без web-view терминологии;
