...
- [ ] `Reader Adjacent Article Transition Animation`: добавить анимацию при переходе к следующей или предыдущей статье из `ReaderView`, чтобы смена article ощущалась как направленная прокрутка / slide по текущему adjacent article direction, а не как резкая замена контента;
