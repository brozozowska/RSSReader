...
- [ ] `Sources Folder Visibility By Filter`: изменить видимость папок на экране `Sources`: при фильтре `All Items` показывать все папки, а при фильтрах `Unread` и `Starred` скрывать пустые для текущего фильтра папки;
