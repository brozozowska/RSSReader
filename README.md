...
- [ ] `Article Retention Setting`: добавить настройку срока хранения архивных статей с вариантами `Current Feed Only`, `2 Days`, `1 Week`, `2 Weeks`, `1 Month`, сохранить её в `AppSettings` и показать в `Settings Screen`;
- [ ] `Article Retention Cleanup`: добавить cleanup архивных статей по retention policy после manual/background refresh и при сохранении настройки; retention считать от `archivedAt`, не от `publishedAt`, а starred статьи не удалять автоматически, пока звезда установлена;
- [ ] `Article Sync Scope Migration`: без поддержки старой модели данных перевести article payload в sync-backed scope или ввести sync-backed article payload model после проверки SwiftData/CloudKit constraints, чтобы архивные, unread и starred статьи материализовались на втором устройстве вместе с `ArticleState`, но изображения оставались локальным cache;
