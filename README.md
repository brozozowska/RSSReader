...
- [ ] `Source Discovery Activity Indicator Alignment`: проверить индикатор поиска / preview источника в `SourceManagementScreen`: если общий custom refresh symbol применяется только к pull/manual refresh, оставить search/preview на UIKit activity indicator; если символ становится общим activity indicator, заменить текущий индикатор без изменения validation и feed discovery flow;
