...
- [x] `Reader Swipe Boundary Navigation`: сделать переход к следующей / предыдущей статье из `ReaderView` зависимым от scroll boundary: первый вертикальный жест должен штатно прокручивать текущую статью к концу или началу, а переключение article должно происходить только при продолжении drag / overscroll за границей контента и только если в текущем article-list context есть соседняя статья;
