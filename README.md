...
- [ ] `Reader Swipe Boundary Navigation`: вернуть swipe / overscroll переход к следующей и предыдущей статье поверх стабилизированного adjacent article flow: обычный вертикальный жест должен сначала прокручивать текущую статью, а переключение должно происходить только после достаточного pull за верхнюю / нижнюю scroll boundary и отпускания пальца, используя тот же app-level adjacent article path, что и toolbar controls;
