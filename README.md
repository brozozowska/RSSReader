...
- [ ] `Article List Refresh Without List Identity Reset`: убрать зависимость стабильности refresh от `articleListIdentity`: manual refresh должен работать поверх существующего `List`, не сбрасывать его identity, не очищать display snapshot во время `.refreshable` и применять результат одним commit после завершения refresh pipeline;
