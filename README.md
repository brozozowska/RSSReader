...
- [ ] `Feed Unsubscribe Confirmation`: добавить предупреждение перед отпиской от ленты из sidebar context menu: destructive action должен сначала создать pending confirmation state с названием ленты, а `AppActionRouter.unsubscribeFeed` вызываться только после подтверждения; отмена не меняет selection, sidebar/article reload IDs и badge state; покрыть sidebar/app-flow tests;
