...
@Suite("Feed Management / Screen Controller / Folder Flow")
@MainActor
struct FeedManagementScreenControllerFolderFlowTests {
    @Test
    func feedManagementScreenControllerReturnsToAddFeedWithNewFolderSelectedAfterInlineFolderCreation() async throws {
        ...
        #expect(previewBeforeFolderDestination.primaryActionTitle == FeedManagementLocalization.addFeedTitle)
        ...
        #expect(addFeedDestination.primaryActionTitle == FeedManagementLocalization.addFeedTitle)
        #expect(addFeedDestination.isConfirmationActionEnabled)
        #expect(addFeedDestination.preview?.title == "Example Feed")
        #expect(addFeedDestination.placementOptions.map(\.title) == [FeedManagementLocalization.ungroupedTitle, "News", "Research"])
        #expect(addFeedDestination.placementOptions.last?.isSelected == true)
    }

    @Test
    func feedManagementScreenControllerRestoresPreviewedAddFeedAfterBackingOutOfNestedFolderCreation() async throws {
        let feedURL = "https://example.com/feed.xml"
        let harness = try TestHarness.make(
            httpClient: ScriptedHTTPClient(
                responsesByURL: [
    ...
    @Test
    func feedManagementScreenControllerCreatesFolderThroughServiceAndRefreshesDestination() throws {
        ...
        #expect(createdDestination.placementDescription == FeedManagementLocalization.existingFolderPlacementDescription(count: 2))
    ...
    @Test
    func feedManagementScreenControllerCreatesFolderRequestsSidebarReloadInAppFlow() throws {
        let harness = try TestHarness.make(httpClient: ScriptedHTTPClient())
        let controller = FeedManagementScreenController()
        ...
        #expect(createdDestination.feedback?.title == FeedManagementLocalization.folderCreatedTitle)
        #expect(createdDestination.feedback?.detail == FeedManagementLocalization.folderCreatedDetail("Research"))
